...
def calc_weights(lam_da, x, y):
    ...
    #     w_first_exp = np.matmul(x_trans, x)
...
def calc_val_err(w, lam_da, test_x, test_y):
    ...
    pred_y = np.matmul(w_trans, np.transpose(test_x))
    #     print test_y.shape
    ...
    #     average_square_error = (square_error[0] / float(pred_y.shape[0] * 2))
...
input = os.path.join('Least-Square-Regression', 'Model_Trees_Full_Dataset', 'preprocessed_datasets.csv')

input_df = pd.read_csv(input)

lambdas = [0, 0.01, 0.001, 0.0001, 0.00001, 0.000001, 0.1, 1, 10, 100, 1000]
# lambdas = [0.01, 0.1]
...
y_df = df.filter(["sum_7yr_GP"])
x_df = df.drop(["sum_7yr_GP"], axis=1)
y_test = testing_df.filter(["sum_7yr_GP"])
...
for i in lambdas:
    ...
    while k_fold <= 10:
        ...
        lambda_vector[i].append(validationErrorSet[k_fold][i][0])
        ...
        k_fold += 1
    # print i
    #     print lambda_vector[i]
    print np.mean(lambda_vector[i])
...
print len(lambda_vector)

avg_train_loss = [(key, np.mean(lambda_vector[key])) for key, value in lambda_vector.iteritems()]
sorted_by_first = sorted(avg_train_loss, key=lambda tup: tup[0])
xdata = []
ydata = []
for x, y in sorted_by_first:
    if x != 0:
        xdata.append(x)
        ydata.append(int(y))

test_loss = [(key, value) for key, value in test_result.iteritems()]
test_loss_sorted = sorted(test_loss, key=lambda tup: tup[0])
xtestdata = []
ytestdata = []
for x, y in test_loss_sorted:
    if x != 0:
        xtestdata.append(x)
        ytestdata.append(int(y))
        # titles and stuff ...
...
print ytestdata
plt.semilogx(xdata, ydata)
plt.semilogx(xtestdata, ytestdata)
