...
def get_LSE(lam_da, x, y, test_x, test_y):
    ...
    # get weights for x, y and lam_da
...
df = input_df[input_df[u'DraftYear'].isin([2004, 2005, 2006, 2007])]
...
for col in x_df.columns:
    if col not in ["country_group", "Position"]:
        ...
        x_df[col] = x_df[col].apply(lambda x: (x - col_mean) / col_std)
...
df.drop(['country_group', 'Position'], inplace=True, axis=1)
